...
**게시판 CRUD, 도보/자전거 폴리라인, 쿠폰(Redis 선착순) 기능은 이번 스프린트 범위에서 완전히 제외한다.**
(쿠폰은 명세서/CLAUDE.md 어디에도 없던 기능이라 "여유 시" 조건부로도 두지 않고 드롭 — 서비스 취지와 무관하고 비회원 원칙과도 충돌 소지가 있음)

남기는 범위: **경로 검색 + 스코어링 + 비교(최단시간 vs 추천) + 따릉이 대여소 조회** 4가지.
...
### 7.2 Q3 — 정차순번 반영: 계단식 대신 완만한 감산으로 변경 제안

원안(B): `stop_sequence ≤ 3`이면 점수 × 0.7, 아니면 그대로(계단식 컷오프).

**A 제안(이 문서에서 처음 제기 — B 확인 필요):** 계단식보다 **거리에 비례해 완만하게 줄어드는 감산**이 더
현실적이다. 정차순번 3에서 4로 넘어가는 순간 보너스가 뚝 끊기는 것보다, 출고역(정차순번 0, "계단 내려오면
바로 있는 빈 열차"에 해당하는 기준점)에서 멀어질수록 보너스가 서서히 사라지는 게 실제 혼잡 누적 양상에 가깝다.
...
- **⚠️ 김재우님 확인 필요**: `bus_weight` 테이블에 `stop_sequence` 컬럼이 현재 계약(§4)에 없음. 원천 데이터가
  이미 `sttn_seq`를 주므로 컬럼 추가가 어렵지 않을 것 — 시간 되면 추가 요청.

### 7.3 Q4 관련 추가 발견 — 버스는 반경 매칭 대신 ID 직접 매칭 가능할 수 있음

`odsay_result.json` 실제 응답을 까본 결과, 버스 구간(`trafficType:2`)에 `localStationID` 필드가 있고
값 형태가 `"118000070"` 식이다. 이게 `bus_stop.stop_std_id`(표준버스정류장ID)와 같은 체계면 **50m 반경
매칭 없이 `stop_std_id = localStationID`로 정확히 조인 가능** — 반경 매칭보다 오매칭 위험이 없다.
`서울시 노선 정류장마스터 정보.csv`의 표준ID 컬럼과 대조해서 확인 필요 (5분 작업).
지하철은 ODsay의 `stationID`가 자체 내부 ID라 이 방식이 통하지 않음 — 기존대로 좌표 매칭 필요.

---

## 8. 개인정보 처리 — `route_request` 충돌 해결안

CLAUDE.md §13(구현 보류) vs 핸드오프 §3(담당 업무)이 충돌하던 부분. 다음 안으로 진행:
**좌표를 소수점 3자리로 절삭 저장**(약 100m 격자)하고, N-04 문구를 "요청 좌표는 일반화하여 저장하며
사용자 식별자와 결합하지 않는다"로 수정. 김재우님께도 공유 필요 (⚠️ 아직 전체 팀 확정 아님).

`/system/meta` 필수 표시 항목이 6개(Front version, Server version, 서버 IP, 서버명, 클라이언트 IP,
X-Forwarded-For)라는 언급이 있었으나 CLAUDE.md §12는 4개만 명시 — 어느 쪽이 맞는지 확인 필요.
...
1. ODsay API 키 확보 여부 (A의 유일한 블로커)
2. §7.2 Q3 계단식→완만한 감산 변경안 — B 동의 여부
3. §7.2 bus_weight에 stop_sequence 컬럼 추가 — 김재우님 확인
4. §7.3 버스 `localStationID` = `stop_std_id` 여부 — 확인되면 매칭 로직 단순화
5. §8 개인정보 처리안(좌표 절삭) — 김재우님 포함 팀 전체 확정 여부
6. §8 `/system/meta` 필드 6개 vs 4개 — 어느 쪽이 맞는지
