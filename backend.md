...
4. §6.1 지하철 노선 곡선 데이터(OSM) 통합 — 두 가지 정해야 함:
   (a) 이 지도 시각화 작업(카카오맵 연동, Polyline 렌더링)을 A/B가 프론트 코드까지 직접 짜는 건지, 데이터만
       넘기고 윤상은님이 통합하는 건지 — 아직 확인 안 됨
   (b) 구간 자르기(시작역~끝역 사이만 추출)를 백엔드 API로 제공할지, 프론트가 전체 노선 GeoJSON을 받아서
       클라이언트에서 자를지
