...
| DB | **⚠️ 확인 필요 — 아래 참고** | ORM 사용 금지 (SQLAlchemy ORM, Prisma 등 X) → 원시 SQL 직접 작성 |
| 캐시 | Redis | ODsay 응답 캐시(동일 출발/도착/시간대 재요청 시 TTL 캐시), 따릉이 실시간 재고 캐시 — **로그인 세션 용도는 없음** (§4 참고) |
...
> **⚠️ DB 종류 미확정 — 팀 확인 필요:** 부트캠프 과제 공통 지침은 **MySQL 8 + 원시 SQL**이었다. 그런데 4조 최종 명세서 §7.2 네트워크 구성도·보안그룹 표는 DB 계층을 **PostgreSQL(5432/tcp)** 로 그리고 있다. 둘 중 하나로 통일해야 코드/인프라가 어긋나지 않는다. 이 문서는 일단 과제 공통 지침을 따라 **MySQL 8**을 기본값으로 유지하되, 팀이 PostgreSQL로 확정하면 이 표와 §9, §10, 커넥터 라이브러리(`mysql-connector-python`/`PyMySQL` → `psycopg2`/`asyncpg`)를 함께 바꿔야 한다. **어느 쪽으로 확정됐는지 확인 후 이 섹션을 갱신할 것.**
>
> ORM을 안 쓰는 이유: SQL 쿼리는 모듈별 `queries.py`에 직접 작성하고, 커넥션 관리(pool)는 공용 코드(`backend/app/core/db.py`)에서 제공한다.
...
- **⚠️ 팀 확인 필요:** 4조 최종 ERD(§9)에는 `route_request`(출발/도착 좌표 원본 + 요청시각), `route_candidate`(추천 결과 로그) 테이블이 포함돼 있다. 이는 스코어링 결과를 검증·디버깅하려는 목적으로 보이지만, **좌표 원본을 저장한다는 점에서 N-04("위치 이력을 저장하지 않는다")와 정면으로 충돌한다.** 코드를 작성하기 전에 팀과 합의가 필요하다:
  - (a) 두 테이블을 아예 빼고 무상태로 처리하거나,
  - (b) 좌표 대신 구/동 단위로 일반화한 값만 저장하거나,
  - (c) 짧은 TTL(예: 24시간) 후 자동 삭제되는 형태로만 두거나.
  - 확정 전까지 Claude는 이 두 테이블을 실제로 생성/저장하는 코드를 작성하지 않는다.
...
- **DB 접근**: 커넥션 풀은 `backend/app/core/db.py`에서만 생성. SQL 인젝션 방지를 위해 파라미터 바인딩(`%s` 또는 `%(name)s`) 필수.
...
| `route_request` | ⚠️ 사용자 요청 원본(출발/도착 좌표, 요청시각) 로그 | §4의 개인정보 원칙과 충돌 — 팀 합의 전까지 실제 구현 보류 |
| `route_candidate` | ⚠️ 추천 결과·스코어링 로그, `request_id`에 INDEX | 위와 동일 사유로 보류 |

사용자별 요청 이력을 영속 저장하는 것은 원래 설계 의도(§4)와 어긋나므로, `route_request`/`route_candidate`는 **디버깅/검증용 임시 로그로 한정**하거나 팀 논의 후 범위를 재정의한다 (§4 참고).
...
| DB 포트 | 명세서 보안그룹 표는 `5432/tcp`(PostgreSQL) 기준 — §2의 DB 종류 미확정 이슈와 함께 확인 필요 |
...
Front version, Server version, 서버 IP, 서버명을 화면 하단 공용 배너로 항상 노출한다. 백엔드 `GET /api/system/version` 엔드포인트에서 값을 받아온다.
...
- DB 접근 코드에 ORM을 사용하지 않는다. 원시 SQL + 파라미터 바인딩만 사용한다.
- **§2의 MySQL vs PostgreSQL 확정 여부를 먼저 확인한다.** 확정되지 않은 상태에서 특정 DB 전용 문법(예: `AUTO_INCREMENT` vs `SERIAL`)을 임의로 고정하지 않는다.
