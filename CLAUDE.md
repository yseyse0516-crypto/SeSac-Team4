...
| API/데이터 | 호출 시점 | 규칙 |
|---|---|---|
| ODsay LAB 경로탐색 API | 사용자 요청 시 | **요청 1건당 1회만 호출**. ⚠️ 무료 한도는 문서상 1,000건/일이었으나 2026-08-19 밤 실제 라이브로 40회 연속 호출해서 **30건/일**임을 확인함(29번째 성공, 30번째부터 `Daily quota exceeded`) — 이 표는 실측값으로 정정. 동일 조건 재요청은 캐시로 흡수해 호출 자체를 줄인다(현재 프로세스 메모리 캐시, backend/app/services/odsay_client.py) |
...
- **`route_request`/`route_candidate` 저장 방식 재확정 (2026-08-20, 최종):** 2026-08-19엔 4조 최종 ERD(§9)의 이 두 테이블을 좌표 소수점 3자리 절삭(약 100m 격자) 저장으로 Postgres에 구현하기로 했었다. 그런데 `favorites`(즐겨찾기)가 "사용자가 다시 보고 싶은 결과"의 영구 보관을 이미 전담하고 있다는 걸 확인한 뒤, 이 두 테이블 자체를 Postgres에서 없애고 **Redis TTL(`route:request:{id}`, 1시간)**로 전환했다(김창영). 좌표는 절삭도 없이 아예 저장하지 않아 N-04를 더 엄격히 지킨다. 상세 내용은 `backend.md` §8/§13 참고.
...
| `users` | 회원 계정(2026-08-19 저녁 추가) | `username` UNIQUE, 비밀번호는 bcrypt 해시만 |
| `board_post` | 커뮤니티 게시글(2026-08-19 저녁 추가) | `owner_user_id` FK |
| `coupon` | 선착순 쿠폰 정의(조건부 기능) | 재고 차감은 Redis에서 원자적으로 처리 |
| ~~`route_request`~~ | (2026-08-20 폐기) 사용자 요청 로그 | Redis `route:request:{id}` TTL 1시간으로 대체, §4 참고 |
| ~~`route_candidate`~~ | (2026-08-20 폐기) 추천 결과·스코어링 로그 | 위와 같은 Redis 키에 포함 |

`route_request`/`route_candidate`는 Postgres 테이블로 두지 않는다 — `favorites`가 영구 보관을 전담하므로 검색 로그는 Redis TTL로만 유지하고 좌표도 저장하지 않는다 (§4 참고). `favorites` 테이블은 아직 Postgres 스키마에 없음(현재 A 쪼 프로세스 메모리 임시 구현, backend.md §12/§13 참고 — 통합 시 추가 필요).
...
- **§4의 개인정보 원칙을 최우선으로 지킨다.** route_request/route_candidate는 Postgres 테이블이 아니라
Redis TTL(`route:request:{id}`, 1시간)로 저장하고, 좌표는 절삭 없이 아예 저장하지 않는다
(2026-08-20 갱신, N-04 대응 — favorites가 영구 보관을 전담해서 이 두 테이블 자체가 불필요해짐).
