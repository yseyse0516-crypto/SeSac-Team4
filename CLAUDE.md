...
> 2026-08-20: `route_request`/`route_candidate`를 Postgres 테이블(2026-08-19 확정)에서 **Redis TTL 저장**으로 다시 전환했다(김창영/백엔드 B). `favorites`가 영구 보관을 전담하는 걸 확인한 뒤라 이 두 테이블은 애초에 영구 저장될 이유가 없었고, 좌표도 절삭 없이 아예 저장하지 않아 N-04를 더 엄격히 지킨다. §4·§9·§13 갱신.
...
- **`route_request`/`route_candidate` 저장 방식 확정 (2026-08-20, 갱신):** 4조 최종 ERD(§9)의 이 두 테이블은 원래 좌표 원본을 저장해 N-04와 충돌 소지가 있었고, 2026-08-19엔 좌표를 소수점 3자리로 절삭(약 100m 격자)해 Postgres에 저장하는 것으로 한 차례 확정했었다. 이후 `favorites`가 "사용자가 다시 보고 싶어하는 결과"를 영구 보관으로 이미 전담하고 있다는 걸 확인해, 이 두 테이블 자체를 Postgres에서 없애고 **Redis TTL(`route:request:{id}`, 1시간)**로 전환했다 — 좌표는 절삭도 없이 저장하지 않고 스코어링에만 쓰고 버린다. Postgres는 그대로 메인 DB다(마스터/가중치/`users`/`board_post`/`coupon`은 영향 없음). 상세 내용은 `backend.md` §8, `docs/decisions/backend-b.md` 4번 항목 참고.
...
| ~~`route_request`~~ | (2026-08-20 폐기) 사용자 요청 로그 — Redis `route:request:{id}`로 대체, 좌표 미저장 | §4 참고 |
| ~~`route_candidate`~~ | (2026-08-20 폐기) 추천 결과·스코어링 로그 — 위와 같은 Redis 키에 포함 | §4 참고 |

`route_request`/`route_candidate`는 Postgres 테이블로 두지 않는다. `favorites`가 영구 보관을 전담하므로, 검색 로그는 Redis TTL(1시간)로만 유지하고 좌표도 저장하지 않는다 (§4 참고).
...
- **§4의 개인정보 원칙을 최우선으로 지킨다.** `route_request`/`route_candidate`는 Postgres 테이블이 아니라 Redis TTL(`route:request:{id}`, 1시간)로 저장하고, 좌표는 절삭 없이 아예 저장하지 않는다 (2026-08-20 갱신, §4 참고).
